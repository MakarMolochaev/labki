...
std::vector<std::unique_ptr<Triangle>> LoadOBJ(const std::string& filename, Material mat, Vector3 delta) {
    ...
    while (std::getline(file, line)) {
        ...
        if (prefix == "v") {
            ...
            vertices.push_back(Vector3(x, y, z) + delta);
        }
        else if (prefix == "f") {
